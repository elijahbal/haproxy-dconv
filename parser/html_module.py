...
def convert_all_html(infiles, outdir, base=''):
    converted = []
    menu = []
    for infile in infiles:
        ...
        data = convert_html(pctxt, infile, outfile, base)
...
def convert_html(pctxt, infile, outfile, base='', version='', haproxy_version=''):
    ...
    for section in section_list:
        ...
        if (title == "Summary") or (title and not hasSummary):
            ...
            document = parser.converter.documentAppend(document, summaryTemplate.render(
                pctxt=pctxt,
                chapters=chapters,
        ...
        if title:
            document = parser.converter.documentAppend(document,
                '<a class="anchor" id="%s" name="%s"></a>' %
                (details["chapter"], details["chapter"]))
            if level == 1:
                document = parser.converter.documentAppend(document, "<div class=\"page-header\">", False)
            ...
            if level == 1:
                document = parser.converter.documentAppend(document, "</div>", False)

        if content:
            if False and title:
                # Display a navigation bar
                document = parser.converter.documentAppend(document, '<ul class="well pager">')
                document = parser.converter.documentAppend(document, '<li><a href="#top">Top</a></li>', False)
                index = chapterIndexes.index(details["chapter"])
                if index > 0:
                    document = parser.converter.documentAppend(document, '<li class="previous"><a href="#%s">Previous</a></li>' % chapterIndexes[index - 1], False)
                if index < len(chapterIndexes) - 1:
                    document = parser.converter.documentAppend(document, '<li class="next"><a href="#%s">Next</a></li>' % chapterIndexes[index + 1], False)
                document = parser.converter.documentAppend(document, '</ul>', False)
            ...
            while pctxt.has_more_lines():
                ...
                if oldline == line:
                    ...
                    if delay:
                        ...
                        document = parser.converter.documentAppend(document, '<pre class="text">%s\n</pre>' % "\n".join(delay), False)
                    delay = []
                    document = parser.converter.documentAppend(document, line, False)
                else:
                    while delay and delay[-1].strip() == "":
                        del delay[-1]
                    if delay:
                        ...
                        document = parser.converter.documentAppend(document, '<pre class="text">%s\n</pre>' % "\n".join(delay), False)
                    delay = []
                    document = parser.converter.documentAppend(document, line, True)
                    pctxt.next()
            ...
            delay = []
            document = parser.converter.documentAppend(document, '</div>')
    ...
    document, keywords, keywordsCount, keyword_conflicts, chapters = \
        parser.converter.createLinks(document, keywords, keywordsCount, keyword_conflicts, chapters)
